...
public class Tienda {
	...
	public boolean anhadeVenta(String id, double importe) throws IOException {//WMC+1
		...
		if (v instanceof VendedorEnPlantilla) {//WMC+1 //CCog+1
			importeFinal += v.getImporte(importeFinal);
		}
	 ...
	 */
	private void vuelcaDatos() throws IOException {//WCA+1
		PrintWriter out = null;
		List<Vendedor> senior = new LinkedList<Vendedor>();
		List<Vendedor> junior = new LinkedList<Vendedor>();
		List<Vendedor> practicas = new LinkedList<Vendedor>();

		for (Vendedor v : lista) {//WCA+1 //CCog+1
			BucleRellenaDatos(senior, junior, practicas, v);
		}
		...
		try {
			...
			out.println();
			out.println("Senior");
			for (Vendedor v : senior) {//WCA+1 //CCog+1
			...
			out.println();
			out.println("Junior");
			for (Vendedor v : junior) {//WCA+1 //CCog+1
			...
			for (Vendedor v : practicas) {//WCA+1 //CCog+1
			...
			if (out != null)//WCA+1 //CCog+1
				out.close();
		}
	}

	private void BucleRellenaDatos(List<Vendedor> senior, List<Vendedor> junior, List<Vendedor> practicas, Vendedor v) {
		if (v instanceof vendedorEnPracticas) {//WCA+1 //CCog+1
			practicas.add(v);
		} else if (v instanceof VendedorEnPlantilla) {//WCA+1 //CCog+1
			VendedorEnPlantilla vp = (VendedorEnPlantilla) v;
			AnhadeVendedorPorTipo(senior, junior, vp);
		}
	}

	private void AnhadeVendedorPorTipo(List<Vendedor> senior, List<Vendedor> junior, VendedorEnPlantilla vp) {//WCA+1
		if (vp.tipo().equals(TipoVendedor.JUNIOR))//WCA+1 //CCog+1
			junior.add(vp);
		else//WCA+1 //CCog+1
			senior.add(vp);
	}
